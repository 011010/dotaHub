    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          {clip.heroId && (
            <span className="text-[10px] text-white/30">Hero #{clip.heroId}</span>
          )}
        </div>
        {clip.streamerName && (
...
export default function SearchPage() {
  ...
  const handleLoadMore = useCallback(async () => {
    if (state.status !== 'done' || state.loadingMore || !state.hasMore) return

    setState(s => s.status === 'done' ? { ...s, loadingMore: true } : s)
  ...
  return (
    <div className="relative min-h-screen">
      <div
        ...
        style={{ background: 'radial-gradient(ellipse at 50% 0%, #1a0a06 0%, #080407 40%, #030203 100%)' }}
      />
          ...
          {hasResults && state.status === 'done' && (
            <div className="mb-6 flex items-center justify-between border-b border-white/[0.06] pb-4">
              <p className="text-sm text-white/50">
                <span className="font-display text-white/90">{state.clips.length}</span>
                {state.total > state.clips.length && (
          ...
          {hasResults && state.status === 'done' && (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {state.loadingMore && Array.from({ length: 3 }).map((_, i) => (
              ...
              <div className="mt-10 flex flex-col items-center gap-3">
                {state.hasMore ? (
                  <button
                    ...
                    className="border border-white/15 bg-white/[0.03] px-12 py-3 font-display text-xs uppercase tracking-[0.35em] text-white/60 backdrop-blur-sm transition-all hover:border-white/30 hover:bg-white/[0.07] hover:text-white/90 disabled:opacity-40"
                  >
                    {state.loadingMore ? (
                      <span className="flex items-center gap-2">
                        <span className="h-3 w-3 animate-spin rounded-full border border-white/60 border-t-transparent" />
                      ...
                      `Load more · ${state.total - state.clips.length} remaining`
                    )}
                  </button>
                ) : (
                  <p className="text-[10px] uppercase tracking-[0.4em] text-white/20">
                    All {state.total} clip{state.total !== 1 ? 's' : ''} loaded
                  </p>
                )}
              </div>
            </>
